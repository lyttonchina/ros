...
# DNS 加速记录：www.betop.vip -> 现有 www.apple-app.cn 主页
...
# 回源规则：www.betop.vip -> www.apple-app.cn
resource "alicloud_esa_origin_rule" "homepage" {
  ...
  rule_name        = "betop-homepage-route"
...
resource "alicloud_esa_cache_rule" "homepage_static" {
  site_id     = local.site_id
  rule_name   = "betop-homepage-static-cache"
  rule_enable = "on"
  rule        = "(http.host eq \"${local.accelerate_domain}\")"
